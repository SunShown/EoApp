...
/**
 * Created by gyliu on 2015/11/27.
 * 改进点：
 * 1.使控件中间显示的时间的上下都有显示，isLoop;
 * 2.原本该控件包括了时间的(时和分)，去掉其xml文件的时和分并且重新调整layout_weght，然后去掉代码中的hour_pv,
 * minute_pv及其引用;
 * 3.让其显示当前时间，每次打开从TextView控件中获取当前日期(SignRankingListActivity、PersonCheckFragment)
 * 4.继续修改3，使其在TeamCheckFragment中在点击时间的上下加减的时候再点击时间控件，使其显示出已经调整过后的日期，
 * 重修show()方法及相应的一系列方法，一直重写到loadComponent()方法
 */
public class TimeSelector_year_month {
    ...
    public TimeSelector_year_month(Context context, ResultHandler resultHandler, String startDate, String endDate) {
    ...
    public TimeSelector_year_month(Context context, ResultHandler resultHandler, String startDate, String endDate, String workStartTime, String workEndTime) {
    ...
    //我写的方法
    public void show_me(int yearInt,int monthInt,int dayInt) {
        if (startCalendar.getTime().getTime() >= endCalendar.getTime().getTime()) {
            Toast.makeText(context, "start>end", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        if (!excuteWorkTime()) return;
        initParameter();
        Log.i("hehe_lgy","show_me:"+yearInt+"-"+monthInt+"-"+dayInt);
        initTimer_me(yearInt,monthInt,dayInt);
        addListener();
        seletorDialog.show();
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            seletorDialog.setContentView(R.layout.dialog_selector_year_month);
            Window window = seletorDialog.getWindow();
            //hehelgy 设置Dialog在屏幕中的显示位置
...
//        minute_pv = (PickerView) seletorDialog.findViewById(R.id.minute_pv);
    ...
    private void initParameter() {
        ...
        spanYear = startYear != endYear;                    //应该是true
        spanMon = (!spanYear) && (startMonth != endMonth);   //应该是false
        spanDay = (!spanMon) && (startDay != endDay);          //应该是false
        spanHour = (!spanDay) && (startHour != endHour);    //应该是false
        spanMin = (!spanHour) && (startMininute != endMininute);    //应该是false
    ...
    private void initTimer_me(int yearInt,int monthInt,int dayInt) {
        initArrayList();
        ...
        loadComponent_me(yearInt,monthInt,dayInt);
...
//        minute_pv.setData(minute);
        //hehelgy--我自己写的  --作用是将点击点击出现时间选择器的初始化时间的TextView的时间
...
//        int mDay = selectedCalender.get(Calendar.DAY_OF_MONTH);
        //Log.i(TAG,mDay+"");
        //相减的数字跟一开始MainActivity设置中的数据有关
        year_pv.setSelected(mYear-1970);
        month_pv.setSelected(mMonth);
  //      day_pv.setSelected(mDay-1);
...
//        minute_pv.setSelected(0);
    ...
    //初始化控件的值
    private void loadComponent_me(int yearInt,int monthInt,int dayInt) {
        year_pv.setData(year);
        month_pv.setData(month);
        day_pv.setData(day);
//        hour_pv.setData(hour);
//        minute_pv.setData(minute);
        ...
        //相减的数字跟一开始MainActivity设置中的数据有关
        Log.i("hehe_lgy",yearInt+"-"+monthInt+"-"+dayInt);
        year_pv.setSelected(yearInt-1970);
        month_pv.setSelected(monthInt-1);
        day_pv.setSelected(dayInt-1);
